...
/*DEPRECATED AS OF 22.10.2015, fn_af_logic used instead.
...
_main_af_task = ["Capture the main airfield","Capture Airfield","testWords",getPos trigger_main_airfield_1,"Search"] call AW_fnc_makeTask;
...
_w_af_task = ["Capture the W airfield","Capture Airfield","testWords",getPos trigger_w_airfield_1,"Search"] call AW_fnc_makeTask;
...
[{
        ...
        if (main_airfield_captured && main_airfield_not_turned) then {
          ...
          //Complete task
          [_main_af_task,"SUCCEEDED"] call BIS_fnc_taskSetState;
        ...
        if (w_airfield_captured && w_airfield_not_turned) then {
          ...
          //Complete task
          [_w_af_task,"SUCCEEDED"] call BIS_fnc_taskSetState;
          //Check if FSM has been started from other AF
          switch mission_fsms_running do {
                case "yes": {
        ...
        if (ne_airfield_captured && ne_airfield_not_turned) then {
          ...
          //Complete task
          [_ne_af_task,"SUCCEEDED"] call BIS_fnc_taskSetState;
          //Check if FSM has been started from other AF
          switch mission_fsms_running do {
                case "yes": {
